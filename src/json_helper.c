...
int parse_json_stream(fhashmap_t *map, const char *infile) 
{
    FILE *fp = fopen(infile, "rb");
    if (!fp) {
        fprintf(stderr, "Failed to open JSON file for streaming parse\n");
        return 0;
    }
    
    stream_buffer_t *sb = init_stream_buffer(CHUNK_SIZE * 2);
    if (!sb) {
        fprintf(stderr, "Failed to initialize stream buffer\n");
        fclose(fp);
        return 0;
    }
    ...
    if (!merged_object) {
        fprintf(stderr, "Failed to create merged JSON object\n");
        success = 0;
        goto cleanup;
    }
